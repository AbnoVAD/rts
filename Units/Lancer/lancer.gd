...
@onready var spear_audio: AudioStreamPlayer = $"sound fx/spear_audio"
@onready var spear_audio_2: AudioStreamPlayer = $"sound fx/spear_audio2"
@onready var shield_audio: AudioStreamPlayer = $"sound fx/shield_audio"

var attack_effect_active:=false # used to prevent multiple spawning

#-------------------------------------------
#HP/SHIELD bar UI
#-------------------------------------------
var ui_visible:=false
var ui_hide_delay:=1.5 #when it's not attack
var ui_timer:=0.0
...
#Stats
#-------------------------------------------
@export var max_life:=230
@export var life:=230

@export var max_guard:=140
@export var guard_stamina:=140
...
func pick_attack_animation()->String:
	return "attack1" if randf()<0.5 else "attack2" #sansa de 50% de a folosi una dintre cele doua animatii

#animation according to the enemy position and direction "attack animation"
...
func play_state(state_name:String):
	match state_name:
		...
		"attack down":
			...
			if not spear_audio.playing:
				spear_audio.play()
		"attack down side":
			animation.play("attack down side")
			if not spear_audio.playing:
				spear_audio.play()
		"attack side":
...
#------------------------------------------
#check for life below 80% before healing
#------------------------------------------
